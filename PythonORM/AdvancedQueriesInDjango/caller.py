...
def product_quantity_ordered() -> str:
    products = Product.objects.values('name').annotate(total_ordered_quantity=Sum('orderproduct__quantity')).exclude(total_ordered_quantity=None).order_by('-total_ordered_quantity')
    return '\n'.join(f"Quantity ordered of {prod['name']}: {prod['total_ordered_quantity']}" for prod in products)
...
def filter_products() -> str:
    filtered_products = Product.objects.filter(Q(price__gt=3.00) & Q(is_available=True)).order_by('-price', 'name')
    return "\n".join(f"{prod.name}: {prod.price}lv." for prod in filtered_products)
...
def give_discount() -> str:
    ...
    Product.objects.filter(query).update(
        price=reduction
    )

    prods = Product.objects.filter(is_available=True).order_by('-price', 'name')
    return "\n".join(f"{prod.name}: {prod.price}lv." for prod in prods)
